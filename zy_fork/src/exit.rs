...
/// A future that never resolves, but will call the handler when it detects a shutsown signal.
///
/// Signals: `SIGINT`, `SIGTERM`, `SIGHUP` or explicit `shutdown` call.
///
/// A use-case of this, is to explicitly shutdown the server. when then cancels this future.
